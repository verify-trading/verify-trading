...
import { BROKER_SYNC_PASSES, type BrokerSyncPass, runBrokerSyncPass } from "@/lib/broker/sync";
import { requireCronSecret } from "@/lib/http/cron-auth";
import { logger } from "@/lib/observability/logger";
import { getSupabaseAdminClient } from "@/lib/supabase/admin";

/**
 * One pg_cron job per pass hits this (BROKER_SYNC_PASSES says when and why). Separate runs
 * because a deploy takes 30 s – 3 min and a serverless function shouldn't sit there waiting for
 * it; runBrokerSyncPass owns what each does.
...
export async function GET(request: Request) {
  ...
  if (!BROKER_SYNC_PASSES.includes(pass as BrokerSyncPass)) {
    return NextResponse.json({ error: `pass must be one of ${BROKER_SYNC_PASSES.join(", ")}` }, { status: 400 });
  }
  ...
  try {
    const outcome = await runBrokerSyncPass(admin, pass as BrokerSyncPass);
