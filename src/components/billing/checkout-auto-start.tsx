...
import type { BillingPromoOfferKey } from "@/lib/billing/promo-offers";
 ...
 */
export function CheckoutAutoStart({
  plan,
  trial = false,
  offer,
}: {
  ...
  useEffect(() => {
    ...
    void (async () => {
      try {
        ...
        toast.error(
          payload?.message ?? "Could not start Stripe checkout.",
        );
        setFailed(true);
      } catch {
