...
export function BillingPageView({
  ...
  const billingDetails = [
    ...
    { label: periodEndLabel, value: renewalDate ?? "—", tabular: true },
  ];
            ...
            <div className="w-full space-y-10">
              ...
              <div className="border-t border-white/10 pt-8">
                <dl className="grid gap-6 sm:grid-cols-3 sm:gap-10">
                  {billingDetails.map(({ label, value, tabular }) => (
                    <div key={label}>
                      <dt className="text-xs font-medium text-[var(--vt-muted)]">{label}</dt>
                      <dd className={cn("mt-1.5 text-base font-semibold text-white", tabular && "tabular-nums")}>
                        {value}
                      </dd>
                    </div>
                  ))}
